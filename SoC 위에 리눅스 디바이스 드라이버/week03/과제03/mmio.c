...
#define LOGGING_FILE      "/tmp/qemu_mmio.log"
...
#define COMENTO_LDR       0x18    /* Log Delete Register */
...
static const unsigned char comento_mmio_id[] = {0x01, 0xa0, 0x1a, 0x00,
	0x0d, 0xf0, 0x05, 0xb1};
*/
// Perpheral ID �~@~S 0x001A_A002, Cell ID 0xB105_F00D
static const unsigned char comento_mmio_id[] = {0x02, 0xa0, 0x1a, 0x00,
        0x0d, 0xf0, 0x05, 0xb1};
                        ...
                        uint64_t value, unsigned size)
{
	...
	if (!comento_fifo_is_full(&s->transmit)) {
	    ...
	    if(s->log_file) {
		    fprintf(s->log_file, "%c", (char)value);
		    fflush(s->log_file);
	    }
        }
...
static void comento_mmio_init(Object *obj)
{
    ...
    memory_region_init_io(&s->iomem, obj, &comento_mmio_ops, s, "mmio-comento", 0x1000);
...
//    s->flag = COMENTO_FLAG_TXFE | COMENTO_FLAG_RXFE;
    s->flag = COMENTO_FLAG_TXFE;

//    object_property_add_str(obj, "data", comento_get_data, comento_set_data);
    
    s->log_file = fopen(LOGGING_FILE, "a");
}
