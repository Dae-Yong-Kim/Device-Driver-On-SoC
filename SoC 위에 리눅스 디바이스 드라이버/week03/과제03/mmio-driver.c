...
#define COMENTO_LDR       0x18    /* Log File Delete Register*/
...
#define COMENTO_IOCTL_REMOVE    _IO(COMENTO_MAGIC, 0)
...
static int comento_dma_init(struct device *dev, const char *name, struct dma_slave_config *conf, struct comento_dma_buf *dma)
{
...
{
	struct comento_mmio_device *cmdev = container_of(work, struct comento_mmio_device, rx_dma_work);
...
static int comento_dma_probe(struct amba_device *adev, struct comento_mmio_device *cmdev)
{
	int ret;
	unsigned long flag;
...
static long comento_mmio_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
	struct comento_mmio_device *cmdev;
	...
	switch(cmd) {
		case COMENTO_IOCTL_REMOVE:
			write_lock(&cmdev->lock);
			writel(COMENTO_INT_RX, cmdev->base + COMENTO_LDR);
...
//        .read = comento_mmio_read,
	...
	.unlocked_ioctl = comento_mmio_ioctl,
};
...
static void comento_mmio_rx_work_handler(struct work_struct *work)
{
	...
	DMA_DEV_TO_MEM,
	MA_PREP_INTERRUPT | DMA_CTRL_ACK);
	if (!desc) {
